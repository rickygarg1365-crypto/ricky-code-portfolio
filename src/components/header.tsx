      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <>
            ...
            {/* Mobile Menu */}
            <motion.div
              className="fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-white shadow-2xl z-50 md:hidden mobile-menu"
              initial={{ x: '100%' }}
              animate={{ x: 0 }}
              exit={{ x: '100%' }}
                  ...
                  {navItems.map((item, index) => (
                    <motion.button
                      ...
                      className={cn(
                        ...
                        item.isContact
                          ? 'bg-primary-500 hover:bg-primary-600 text-white px-6 py-4 rounded-xl font-bold text-lg flex items-center justify-between'
                          : 'text-gray-700 hover:text-primary-600 py-3 text-lg font-medium border-b border-gray-100 hover:border-primary-200'
                      )}
                    ...
                    >
                      ...
                      {!item.isContact && (
                        <motion.div
                          className="w-0 h-0.5 bg-primary-500 rounded-full group-hover:w-full transition-all duration-300"
                        />
                      )}
                    </motion.button>
                  ))}
